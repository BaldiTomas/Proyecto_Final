...
export function ProductList({
  products, loading, userRole, onDeleteProduct, onSelectProduct, onUpdateProduct,
}: ProductListProps) {
  ...
  function handleFilterChange<T extends keyof typeof filters>(key: T, value: typeof filters[T]) {
  ...
  if (filters.stock === "asc") filtered = filtered.slice().sort((a, b) => Number(a.stock) - Number(b.stock));
  if (filters.stock === "desc") filtered = filtered.slice().sort((a, b) => Number(b.stock) - Number(a.stock));
    ...
    <section className="space-y-6">
          ...
          <DialogContent className="bg-slate-800 border-slate-700 max-w-sm space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <Select
                  value={filters.stock}
                  onValueChange={val => handleFilterChange("stock", val as "none" | "asc" | "desc")}
                >
                  <SelectTrigger className="bg-slate-700 border-slate-600 text-white w-full">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading
          ...
          : filtered.map((p, idx) => {
            const statusKey = p.shipment_status && STATUS_LABELS[p.shipment_status] ? p.shipment_status : "default";
            return (
              <Card
                key={`${p.id}-${idx}`}
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex flex-col space-y-1">
                      <span className={`self-start mb-2 px-2 py-1 rounded text-xs font-semibold ${STATUS_STYLES[statusKey]}`}>
